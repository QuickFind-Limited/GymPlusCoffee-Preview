        ...
        <div className="flex h-full flex-col">
          ...
          <div className="px-6 py-4 border-b bg-background/95 supports-[backdrop-filter]:bg-background/60 backdrop-blur sticky top-0 z-10 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {fileKind === "excel" && excelData.length > 1 && (
                <span className="text-xs text-muted-foreground">
                  {excelData.length - 1} rows
                </span>
              )}
              <Button
                variant="outline"
          ...
          <div className="px-6 pb-6 pt-4 flex-1 min-h-0 overflow-hidden">
            ...
            {!loading && !error && (
              fileKind === "pdf" ? (
                <div className="h-full rounded border bg-muted/10">
                  ...
                  {fileKind === "markdown" ? (
                    ...
                    <div className="p-4 overflow-auto">
                      {excelData.length > 0 ? (
                            ...
                            {excelData.slice(1).map((row, ri) => (
                              ...
                              >
                                {row.map((cell, ci) => (
                                  <td
                                    key={ci}
                                    className="px-3 py-2 border-b border-r align-top whitespace-pre-wrap"
                                  >
                                    {cell ?? ""}
                                  </td>
                                ))}
                                {row.length < excelData[0].length &&
                                  Array.from({ length: excelData[0].length - row.length }).map((_, k) => (
                                    <td
                                      key={`pad-${k}`}
                                      className="px-3 py-2 border-b border-r"
                                    />
                        ...
                        <div className="text-sm text-muted-foreground">
                          Feuille Excel vide ou illisible.
                        </div>
                      )}
                    </div>
                  ) : fileKind === "image" ? (
