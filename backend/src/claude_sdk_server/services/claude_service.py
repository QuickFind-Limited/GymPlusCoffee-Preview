...
class ClaudeService:
    ...
    async def query(self, request: QueryRequest) -> QueryResponse:
        ...
        # Use the session_id if available (for resumed sessions), otherwise use temp ID
        conversation_id_for_prompt = request.session_id if request.session_id else temp_conversation_id
        enhanced_system_prompt = self._build_enhanced_system_prompt(
            request.system_prompt, conversation_id_for_prompt
        )
            ...
            async for message in query(prompt=request.prompt, options=options):
                ...
                if isinstance(message, ResultMessage):
                    try:
                        ...
                        # Fallback: use the LAST collected message only
                        if all_assistant_messages:
                            response_text = all_assistant_messages[-1]
        ...
        try:
            ...
            try:
                ...
                if not response_text and all_assistant_messages:
                    # Only use the LAST assistant message, not all of them
                    response_text = all_assistant_messages[-1]
            except Exception as e:
                logger.error(f"Failed to extract response text: {e}")
                if all_assistant_messages:
                    try:
                        # Only use the LAST assistant message as fallback
                        response_text = all_assistant_messages[-1]
                    except Exception:
            ...
            # Absolute fallback
            try:
                # Only use the LAST assistant message in fallback scenario
                response_text = all_assistant_messages[-1] if all_assistant_messages else "Processing failed"
...
- CRITICAL: Your Python script MUST save the PDF to the attachments folder using this EXACT pattern:
  ```python
  import os
  script_dir = os.path.dirname(os.path.abspath(__file__))
  attachments_dir = os.path.join(os.path.dirname(script_dir), 'attachments')
  os.makedirs(attachments_dir, exist_ok=True)
  output_path = os.path.join(attachments_dir, 'report.pdf')
  ```
- NEVER use the conversation_id variable inside Python scripts
- NEVER create nested tmp directories - the structure is: tmp/{conversation_id}/utils/ and tmp/{conversation_id}/attachments/
- Write the Python script in utils folder, execute it, and it will save PDF to attachments folder
...
- Files in the attachments folder will be automatically available for download in the UI
"""
            logger.info(f"Enhanced prompt with conversation_id: {conversation_id}")
            if original_prompt:
                return original_prompt + file_organization_instructions
            else:
                # No default prompt - only add file organization instructions
                return file_organization_instructions
